@echo off
:: ════════════════════════════════════════════════════════════════════════
::  StudyBuddy  ·  Windows .exe Build Script
::  Run from the project root:   build_windows.bat
::  Output folder:               dist\StudyBuddy\
:: ════════════════════════════════════════════════════════════════════════

echo.
echo ========================================
echo  StudyBuddy  Build Script
echo ========================================
echo.
...
if exist "dist\StudyBuddy" (
    taskkill /F /IM "StudyBuddy.exe" 2>nul
    timeout /t 1 /nobreak >nul
    rmdir /S /Q "dist\StudyBuddy" 2>nul
)
...
%PYTHON% -m PyInstaller ^
  ...
  --add-data "play.html;." ^
  --add-data "STUDYBUDDY_SUBJECT_TEMPLATE.json;." ^
  --collect-all "uvicorn" ^
  --collect-all "webview" ^
...
robocopy "subjects" "dist\StudyBuddy\subjects" /E /NFL /NDL /NJH /NJS /NC /NS 1>nul
if %errorlevel% GEQ 8 (
    echo     WARNING: robocopy failed, creating empty subjects folder instead.
    mkdir "dist\StudyBuddy\subjects" 2>nul
) else (
...
echo ========================================
echo.
echo  Game folder:  dist\StudyBuddy\
echo  Launch with:  dist\StudyBuddy\StudyBuddy.exe
echo.
echo  To share with playtesters:
echo    Zip the entire  dist\StudyBuddy\  folder.
echo    Players unzip anywhere and run StudyBuddy.exe.
